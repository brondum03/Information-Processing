...
foreach item $ip {
   if {[catch { glob -directory ${item}/solution1/impl/ip/ *.zip} zip_file]} {
      ...
      ### Y2K22 HOTFIX ###
      set ippack_file "${item}/solution1/impl/ip/run_ippack.tcl"
      if {[file exists $ippack_file]} {
         # Read file
         set fd [open $ippack_file r]
         set content [read $fd]
         close $fd

         set content [regsub {set Revision\s+\d{10,}} $content "set Revision 2147483647"]

         # Write back
         set fd [open $ippack_file w]
         puts $fd $content
         close $fd

         puts "  -> Patched Y2K22 bug in $ippack_file"
      }
   } else {
